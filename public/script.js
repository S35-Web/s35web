...
(function() {
    const LANGUAGE_PREFERENCE_KEY = 's35_language_preference';
    const MODAL_SHOWN_KEY = 's35_modal_shown';
    
    function initLanguageModal() {
        const modal = document.getElementById('languageModal');
        const continueGlobalBtn = document.getElementById('continueGlobalBtn');
        const goToMexicoBtn = document.getElementById('goToMexicoBtn');
        
        if (!modal || !continueGlobalBtn || !goToMexicoBtn) return;
        
        // Check if user has already made a choice or seen the modal
        const preference = localStorage.getItem(LANGUAGE_PREFERENCE_KEY);
        const modalShown = localStorage.getItem(MODAL_SHOWN_KEY);
        
        // If preference exists, don't show modal
        if (preference === 'global' || preference === 'mexico') {
        ...
        // Continue in Global Page
        continueGlobalBtn.addEventListener('click', () => {
            localStorage.setItem(LANGUAGE_PREFERENCE_KEY, 'global');
            localStorage.setItem(MODAL_SHOWN_KEY, 'true');
            closeModal();
        });
        
        // Go to S-35 Mexico Page
        goToMexicoBtn.addEventListener('click', () => {
            localStorage.setItem(LANGUAGE_PREFERENCE_KEY, 'mexico');
            localStorage.setItem(MODAL_SHOWN_KEY, 'true');
            window.location.href = 'https://www.S-35.com.mx';
        });
        ...
        // Prevent closing by clicking backdrop (force user to choose)
